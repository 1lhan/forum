...
router.post('/register', async (req, res) => {
    ...
    const user = new UserModel({
        username, email, password: hashedPassword, accountType: 'user', membershipDate: new Date(),
        bookmarks: [], comments: [], myTopics: [], reactions: [], profileImage: ''
    })
    let register = await user.save()

    return res.json({ success: register ? true : false, msg: register ? '' : 'Account could not create' })
})
...
router.post('/change-password', async (req, res) => {
    ...
    return res.json({ success: update ? true : false, msg: update ? '' : 'Password could not be changed' })
})
